...
| 메서드       | 호출 주기                        | 주 사용 용도                      | 주의점                                               |
|--------------|----------------------------------|----------------------------------|------------------------------------------------------|
| Update()     | 매 프레임마다 호출               | 게임 로직, 사용자 입력 처리        | 프레임 레이트에 따라 호출 빈도가 달라질 수 있음       |
| FixedUpdate()| 고정된 시간 간격마다 호출       | 물리 연산 처리                     | 일정한 주기로 호출되며, 물리 연산의 정확성 보장       |
| LateUpdate() | Update() 이후에 호출             | 카메라 위치 업데이트, 후처리 로직 | Update() 이후 실행되므로 오브젝트의 최종 상태에 접근 가능 |
